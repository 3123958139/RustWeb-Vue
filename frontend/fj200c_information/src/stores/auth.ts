/**
 * 发动机监控应用（fj200c_information）认证 Store
 *
 * 使用 @shared 工厂函数创建，仅放行 fj200c_information 角色。
...
export const useAuthStore = createAuthStore({
  id: "auth-fj200c_information",       // Pinia Store 唯一标识符
  appKind: "user",          // 应用类型（user 表示用户端）
  allowedRoles: ["fj200c_information"], // 仅允许 fj200c_information 角色登录
  authApi,                  // 认证 API 实例
});
