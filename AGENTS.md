...
Rust + Axum 后端 + 9 个 Vue 3 前端应用（8 用户端 + admin 管理后台）的全栈管理系统。不是 Tauri 项目。根目录为 npm workspaces，后端 Rust 独立构建。

## 架构

- **后端** `src/` — Axum 0.7 Web 框架，SQLite (sqlx)，JWT 认证，RBAC 角色权限
- **前端** `frontend/` — 9 个独立 Vue 3 + Vite 6 + Element Plus + Pinia 应用，各占不同端口
...
# 一键部署（Windows，根目录）
deploy.bat                   # 9 个前端并行 npm run build（build-frontends.ps1）→ cargo build --release --features embedded（前端嵌入 exe）→ 组装 deploy/ 并启动
```
...
- `src/embedded_assets.rs` — 单 exe 打包：`--features embedded` 时用 rust-embed 将 9 个前端 dist 编译期内嵌，内存服务静态资源
...
- **数据库**：SQLite 文件在运行目录自动创建（开发为根目录 `rustweb.db`，部署为 `deploy/rustweb.db`），无手动安装。建表与种子账号由 `src/database.rs` 内建（无 sqlx 迁移文件）；**种子账号初始密码是随机生成的**（9 个角色，邮箱 `@7304.com`），明文只存 `seed_passwords` 表，经 `GET /admin/pwd` 查询（`src/admin/routes.rs:73` 的 `PUT /api/users/settings/pwd-route` 可停用该端点）
...
- **静态托管**（生产）：后端 `main.rs` 双模式——`embedded` feature 下 9 个前端已内嵌进 exe（`embedded_assets.rs` 内存服务，SPA 深链接回退 index.html）；默认 dev 模式仍读磁盘 `dist-*/` 目录；根路径 `/` 重定向到 `/admin`
...
- **数值类注释必须与实现一致**：防漂移。典型为 `src/api_docs.rs` 的 `export_openapi` 测试断言注释（当前 59 路径 / 74 操作，AGENTS.md 与 README.md 中同样数字需同步维护）与 `src/roles.rs` 模块头的角色列表——改注册表/新增角色后必须一并更新
...
```powershell
deploy.bat        # 1) 9 个前端并行 npm run build（build-frontends.ps1）2) cargo build --release --features embedded 3) 组装 deploy/
```
...
```
deploy/
├── rust-web-backend.exe     # 单文件后端（内嵌 9 个前端 dist，双击即可启动）
├── .env                     # 运行时配置（不存在时自动生成）
...
3. handler 加 `#[utoipa::path]`（tags="xxx"）；`src/api_docs.rs` 追加 paths/schemas/tags，并同步更新 `export_openapi` 测试里的路径数量/操作数量断言（59 路径 / 74 操作，防漂移）
