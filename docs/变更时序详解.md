...
> 涉及事实以当前代码为准（OpenAPI 66 路径 / 83 操作、9 个前端应用、dev 端口 5173–5181）。
...
- **数据库文件位置**：开发在根目录 `fj200c.db`，部署在 `deploy/fj200c.db`，两边结构独立，迁移脚本都要生效。
...
| `DATABASE_URL` | 重启后端 | 默认 `sqlite://fj200c.db` |
| `JWT_SECRET` | 硬编码 | 硬编码进 `src/common/jwt.rs`（`HARDCODED_JWT_SECRET`），无需环境变量；仅用于 JWT 签名，与数据加密密钥独立 |
| `DATA_ENCRYPTION_KEY` | 启动时派生 | 由本机 MAC 地址自动派生（`src/common/crypto.rs:data_key()`），用于库内敏感字段 AES-256-GCM 加解密与 HMAC 指纹；MAC 变更会使旧密文失效（预期行为） |
| `JWT_EXPIRATION` | 重启后端 | 默认 86400 秒 |
| `RUST_LOG` | 重启后端 | 日志级别 |
| `CORS_ORIGINS` | 重启后端 | **release 模式缺失/为空直接拒绝启动**（main.rs CORS 白名单，逗号分隔）；dev 模式放行任意来源 |

**时序要点**：部署环境首次启动 `deploy\.env` 不存在时由 deploy.bat 自动生成（含占位 CORS_ORIGINS）；改端口后重启 exe 前，deploy.bat 会 netstat 检查旧端口占用并询问是否 kill。
...
sequenceDiagram
    ...
    D->>O: 5. start fj200c-backend.exe
```
...
| 3 | 组装 `deploy/`：exe + `.env`（自动生成，含 CORS_ORIGINS 占位）+ 3 个 config ini（不存在则生成默认）+ `csv/` 目录 | 目录结构完整 |
| 4 | 端口占用检查：`netstat -ano` 找 LISTENING，choice 询问是否 kill | 端口空闲 |
| 5 | `start "RustWeb" cmd /k fj200c-backend.exe` | 浏览器访问对应路径 |

**时序要点**：
- 步骤 1 → 2 顺序不可颠倒（内嵌依赖）；步骤 3 的配置文件若已存在则**保留**（不覆盖用户修改）。
- release 模式下 `CORS_ORIGINS` 缺失会拒绝启动——deploy.bat 自动生成的 `.env` 已含占位值，但**生产必须手工改成前端来源白名单**。JWT 签名密钥已硬编码进代码，数据加密密钥由本机 MAC 自动派生，均无需配置。
...
- 部署回滚：`deploy/` 目录直接替换旧 exe（`.env` / `config-*.ini` / `fj200c.db` 保留，不覆盖）；前端 dist 内嵌于 exe，无法单独回滚前端，需重编 exe 或保留旧 exe。
- 数据库回滚：无迁移文件，靠 `ALTER TABLE` 反向语句或删除 `fj200c.db` 重建（种子账号会重新生成随机密码，需重新查 `GET /admin/pwd`）。
...
| `.env` 配置 | — | 重启后端 | release 下缺失 CORS_ORIGINS 拒绝启动（JWT_SECRET 已硬编码、数据密钥由 MAC 派生，均无需配置） |
