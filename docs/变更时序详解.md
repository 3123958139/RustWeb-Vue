...
> 面向 RustWeb-Vue（Rust + Axum 后端 + 8 个 Vue 3 前端）的**变更操作手册**。
> 本文档聚焦每种可能的变更**必须遵守的先后顺序**（时序）：什么必须先做、什么必须后做、每一步如何验证、踩错顺序会怎样。
> 涉及事实以当前代码为准（OpenAPI 49 路径 / 61 操作、8 个前端应用、dev 端口 5173–5180）。
...
1. **前端必须先于后端构建**。`deploy.bat` 把 8 个前端 dist 在**编译期**内嵌进 exe（`src/embedded_assets.rs` 的 rust-embed），后端编译时若 `frontend/<app>/dist/` 不存在会直接编译失败。顺序固定：`build-frontends.ps1` → `cargo build --release --features embedded`。
...
| 步骤 | 操作 | 文件 |
|---|---|---|
| 6.1 | 前端构建列表加一项（两波各 4 个，新增后注意波次平衡） | `build-frontends.ps1` 的 `$apps` |
...
- **种子账号邮箱格式**：`<role>@7304.com`（8 个角色），改名/新增角色时同步。
...
| 步骤 | 动作 | 验证点 |
|---|---|---|
| 1 | `build-frontends.ps1`：8 个应用并行（`Start-Job` 两波各 4 个），任一失败输出 `[FAILED]` 并 exit 1 | 全部 `[OK]` |
...
- 新增角色后，`$apps` 两波分配要重新平衡（当前 4+4，加 1 个变 4+5 或 5+4，`build-frontends.ps1` 按 `$apps[0..3]` / `$apps[4..7]` 硬编码切分，需手动调整）。
...
**时序要点**：
- 7 个用户端 + admin 共享同一 localStorage token，跨应用跳转 token 自动传递；跳转目标由 `ROLE_APP_URLS` 决定（dev 完整 URL / prod 路径前缀）。
...
| 部署 | `deploy.bat` | 8 个前端 `[OK]` → exe 编译 → 启动 |
...
> 维护约定：本文件涉及的数字（49 / 61、8 个应用、端口等）与 AGENTS.md / README.md / `src/api_docs.rs` 注释共享同一事实源，修改时需一并更新。